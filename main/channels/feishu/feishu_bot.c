/**
 * @file feishu_bot.c
 * @brief Feishu Bot Implementation
 * 
 * 基于 WebSocket 长连接接收飞书消息
 *
 * @author MtZhr
...
#include "agent/command_parser.h"
...
/* 凭证和 Token 状态 */
static char s_app_id[64] = LITEARM_SECRET_FEISHU_APP_ID;
static char s_app_secret[128] = LITEARM_SECRET_FEISHU_APP_SECRET;
...
static bool pb_skip_field(const uint8_t *buf, size_t len, size_t *pos, uint8_t wire_type)
{
    uint64_t n = 0;
    switch (wire_type) {
        case 0: return pb_read_varint(buf, len, pos, &n);
...
static bool pb_parse_header_msg(const uint8_t *buf, size_t len, ws_header_t *h)
{
    ...
    while (pos < len) {
        ...
        if (wt != 2) { if (!pb_skip_field(buf, len, &pos, wt)) return false; continue; }
        if (!pb_read_varint(buf, len, &pos, &slen)) return false;
        if (pos + (size_t)slen > len) return false;
        if (field == 1) { size_t n = (slen < sizeof(h->key)-1) ? (size_t)slen : sizeof(h->key)-1; memcpy(h->key, buf+pos, n); h->key[n] = '\0'; }
        else if (field == 2) { size_t n = (slen < sizeof(h->value)-1) ? (size_t)slen : sizeof(h->value)-1; memcpy(h->value, buf+pos, n); h->value[n] = '\0'; }
        pos += (size_t)slen;
    }
    return true;
}
...
static bool pb_parse_frame(const uint8_t *buf, size_t len, ws_frame_t *f)
{
    ...
    while (pos < len) {
        ...
        uint32_t field = (uint32_t)(tag >> 3);
        uint8_t wt = (uint8_t)(tag & 0x07);
        if (field == 1 && wt == 0) { if (!pb_read_varint(buf, len, &pos, &f->seq_id)) return false; }
        else if (field == 2 && wt == 0) { if (!pb_read_varint(buf, len, &pos, &f->log_id)) return false; }
        else if (field == 3 && wt == 0) { if (!pb_read_varint(buf, len, &pos, &v)) return false; f->service = (int32_t)v; }
        else if (field == 4 && wt == 0) { if (!pb_read_varint(buf, len, &pos, &v)) return false; f->method = (int32_t)v; }
        else if (field == 5 && wt == 2) {
            if (!pb_read_varint(buf, len, &pos, &blen)) return false;
            if (pos + (size_t)blen > len) return false;
            if (f->header_count < 16) pb_parse_header_msg(buf+pos, (size_t)blen, &f->headers[f->header_count++]);
            pos += (size_t)blen;
        }
        ...
        else { if (!pb_skip_field(buf, len, &pos, wt)) return false; }
    }
    return true;
}
...
/**
 * @brief 获取/刷新 tenant_access_token
 */
static esp_err_t feishu_get_tenant_token(void)
{
    ...
    if (token && cJSON_IsString(token)) {
        ...
        ESP_LOGI(TAG, "Got tenant access token (expires in %ds)", expire ? expire->valueint : 7200);
    }
...
static void handle_message_event(cJSON *event)
{
    ...
    /* 路由: 私聊用 sender_id, 群聊用 chat_id */
    const char *route_id = (strcmp(chat_type, "p2p") == 0 && sender_id[0]) ? sender_id : chat_id;

    /* 推送到消息总线 */
    litearm_msg_t msg = {0};
    strncpy(msg.channel, LITEARM_CHAN_FEISHU, sizeof(msg.channel) - 1);
...
static void feishu_ws_task(void *arg)
{
    ...
    while (1) {
        ...
        esp_websocket_client_config_t ws_cfg = {
            .uri = s_ws_url,
            .buffer_size = 2048,
            .task_stack = LITEARM_FEISHU_POLL_STACK,
...
esp_err_t feishu_bot_init(void)
{
    /* 从 NVS 加载凭证 */
    nvs_handle_t nvs;
    if (nvs_open(LITEARM_NVS_FEISHU, NVS_READONLY, &nvs) == ESP_OK) {
        char tmp_id[64] = {0};
        char tmp_secret[128] = {0};
        size_t len_id = sizeof(tmp_id);
        size_t len_secret = sizeof(tmp_secret);

        if (nvs_get_str(nvs, LITEARM_NVS_KEY_FEISHU_APP_ID, tmp_id, &len_id) == ESP_OK && tmp_id[0]) {
            strncpy(s_app_id, tmp_id, sizeof(s_app_id) - 1);
        }
        if (nvs_get_str(nvs, LITEARM_NVS_KEY_FEISHU_APP_SECRET, tmp_secret, &len_secret) == ESP_OK && tmp_secret[0]) {
    ...
    if (s_app_id[0] && s_app_secret[0]) {
        ...
        ESP_LOGW(TAG, "No Feishu credentials. Use CLI: set_feishu <APP_ID> <APP_SECRET>");
    }
...
esp_err_t feishu_bot_start(void)
{
    ...
    BaseType_t ok = xTaskCreatePinnedToCore(
        feishu_ws_task,
        "feishu_ws",
        LITEARM_FEISHU_POLL_STACK,
        NULL,
        LITEARM_FEISHU_POLL_PRIO,
        &s_ws_task,
        LITEARM_FEISHU_POLL_CORE);
    if (ok != pdPASS) {
        s_ws_task = NULL;
        return ESP_FAIL;
    }
...
esp_err_t feishu_send_message(const char *chat_id, const char *text)
{
    ...
    while (offset < text_len) {
        ...
        if (chunk > LITEARM_FEISHU_MAX_MSG_LEN) {
            chunk = LITEARM_FEISHU_MAX_MSG_LEN;
        }
...
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(LITEARM_NVS_FEISHU, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, LITEARM_NVS_KEY_FEISHU_APP_ID, app_id));
    ESP_ERROR_CHECK(nvs_set_str(nvs, LITEARM_NVS_KEY_FEISHU_APP_SECRET, app_secret));
